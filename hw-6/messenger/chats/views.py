...
from chats.models import Chat, ChatType, Message, ChatMember, MemberPrivilege
...
def member_serialize(chat_member: ChatMember):
    serialized_member = {
        'id': chat_member.pk,
        'chat_id': chat_member.chat.pk,
        'member_id': chat_member.member.pk,
        'privilige_id': chat_member.privilege.pk,
...
@require_http_methods(['PATCH'])
def message_mark_as_read(request, message_id):
    message = get_object_or_404(Message, id=message_id)
    if message.read:
        return JsonResponse({'message_is_read': True},  status=400)
    message.read = True
    message.save
    marked_message = message_serialize(message)
    return JsonResponse({'marked_message': marked_message}, status=200)
...
def member_create(request):
    ...
    if ChatMember.objects.filter(member=member_id, chat=chat_id).exists():
        return JsonResponse({'member_exist': True},  status=400)
    data['member'] = User.objects.get(id=data['member'])
    data['chat'] = Chat.objects.get(id=data['chat'])
    data['privilege'] = MemberPrivilege.objects.get(id=data['privilege'])
    member = ChatMember.objects.create(**data)
    created_member = member_serialize(member)
    return JsonResponse({'created_message': created_member}, status=201)


@require_http_methods(['DELETE'])
def member_delete(request, member_id, chat_id):
    member_to_delete = get_object_or_404(ChatMember, member_id=member_id, chat_id=chat_id)
    deleted_member = member_serialize(member_to_delete)
    member_to_delete.delete()
    return JsonResponse({'deleted_member': deleted_member}, status=200)
